...
#Set mfrow to 2,2 to add 4 plots on one page. Add inside of png() call to have it plot all 4 plots in one png file.
par(mfrow = c(2,2) )

#Graph each plot.
with(feb, plot(strptime(Times, "%d/%m/%Y %H:%M:%S"), Global_active_power, type = "l", xlab = "", ylab = "Global Active Power"))
with(feb, plot(strptime(Times, "%d/%m/%Y %H:%M:%S"), Voltage, type = "l" , xlab = "datetime") )
with(feb, {
